...
Процедура ВыполнитьЗагрузку() Экспорт
    ...
    Крутилка.Тик("Подключение к базе");
    Крутилка.Тик("Чтение файла конфигурации");
    Крутилка.Тик("Загрузка объектов");
    Крутилка.Остановить("Готово");

КонецПроцедуры

```

Желудь `Крутилка` уже зарегистрирован в `autumn-cli`, поэтому его можно внедрить в команду через `&Пластилин`.
`Тик("Сообщение")` лениво запускает крутилку, если она еще не активна. Пустой `Тик()` вне активной крутилки ничего не делает.

Если нужно гарантированно остановить крутилку вокруг отдельного действия, используйте `ВыполнитьДействие()`.
Действие должно иметь экспортный метод `Исполнить()`.

```bsl

Крутилка.ВыполнитьДействие("Загрузка объектов", Действие, "Готово");

```

В CI и других неинтерактивных окружениях крутилка автоматически включает тихий режим, чтобы не засорять логи управляющими последовательностями.
Для тестов или ручного отключения вывода можно вызвать `УстановитьТихийРежим(Истина)`.
